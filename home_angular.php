  ...
  //alert(response);
    ...
    for (i = 0; i < arr.length; i++) {
      out += "<div class='row' style='color:black'><div class='col-md-3 text-right' " + 
        ...
        wordwrap(arr[i].tweat, {$tweat_width}, '<br />', true);
      if (arr[i].name == '{$name}') {
// X button to delete Tweat
        ...
        out += "&nbsp;&nbsp;<img src='xdel.png' style='position:relative;top:-1px' onclick='if " + 
          ...
          "...\")) {location.replace(\"{$self_name}?delete_tweat=" + arr[i].tid + "\");}' />";
      }
      out += "</p></div></div>";
    }
    //alert(out);
...
EODJ;
  echo "</head><body background='pictures/backviolet.png' onload='getTweatList();' 
...
</div></div></div><div class='row' id='tweats_display'>
EODF;
...
echo <<<EODANG
<div ng-app="" ng-controller="tweatsController">
<ul>
<li ng-repeat="x in tweats">
{{ x.name + ':  ' + x.tweat }}
</li>
</ul>
</div>
...
<script>
function tweatsController(\$scope,\$http) {
  \$http.get("tweats_JSON.php").success(function(response) {\$scope.tweats = response;});
}
</script> 
EODANG;
